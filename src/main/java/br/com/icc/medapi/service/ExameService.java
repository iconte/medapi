...
public class ExameService {
  ...
  public Page<ExameDTO> listarPorCategoria(Long categoriaExameId, int number, int size) {
    ...
    Optional<CategoriaExame> categoria =
        Optional.ofNullable(categoriaExameRepo.findById(categoriaExameId));
  ...
  private Page<ExameDTO> converterPageExameParaDTO(Page<Exame> pageExame) {
    ...
    List<ExameDTO> exameDTOS = pageExame.getContent().stream()
        .map(exame -> mapper.map(exame, ExameDTO.class)).collect(Collectors.toList());
