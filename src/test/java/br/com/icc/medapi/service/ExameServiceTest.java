...
public class ExameServiceTest {
  ...
  public void deveRetornarListaExameslistarTodos() throws Exception {
    ...
    Mockito.when(exameRepo.findAll(req)).thenReturn(mockResult);
  ...
  public void deveRetornarListaExameslistarPorCategoria() throws Exception {
    ...
    categoria.setId(1L);
    Mockito.when(categoriaExameRepo.findById(1L)).thenReturn(categoria);
    Mockito.when(exameRepo.findByCategoriaExame(categoria)).thenReturn(mockListaCategoria);
